...
ds$T1979 <- calibrate(ds$Turnout, thresholds = qT(1979)) %>% round(digits = 4)
ds$T1984 <- calibrate(ds$Turnout, thresholds = qT(1984)) %>% round(digits = 4)
ds$T1989 <- calibrate(ds$Turnout, thresholds = qT(1989)) %>% round(digits = 4)
ds$T1994 <- calibrate(ds$Turnout, thresholds = qT(1994)) %>% round(digits = 4)
ds$T1999 <- calibrate(ds$Turnout, thresholds = qT(1999)) %>% round(digits = 4)
ds$T2004 <- calibrate(ds$Turnout, thresholds = qT(1994)) %>% round(digits = 4)
ds$T2009 <- calibrate(ds$Turnout, thresholds = qT(2009)) %>% round(digits = 4)
ds$T2014 <- calibrate(ds$Turnout, thresholds = qT(2014)) %>% round(digits = 4)
ds$T2019 <- calibrate(ds$Turnout, thresholds = qT(2019)) %>% round(digits = 4)
# Decide which calibration to use based on election year
...
```{r}
ds$E <- ds$Cycle %>% calibrate(thresholds = c(0.25, 0.5, 0.75)) %>% round(digits = 4)
plot(ds$Cycle, ds$E)
```
...
```{r}
ds$S <- ds$EuroscepticVoteShare %>% calibrate(thresholds = c(0, 0.1, 0.3)) %>% round(digits = 4)
...
### Model 4 without national turnout
```{r}
reg3 <- lm(
  data = ds.reg,
  formula = Turnout ~ C + CycleM + H + AgrShare + Year_)
reg3.pcse <- pcse(
  object = reg3,
  groupN = ds.reg$Country,
  groupT = ds.reg$Year,
...
```{r}
qca1.ds <- ds %>% select(T, C, N, E, H, A)
qca1.direxp.high = "1,1,1,1,1"
qca1.direxp.low = "0,0,0,0,0"
qca1.cutoff = ".8"
qca1.method = "QMC"
```
...
### Select variables and directional expectations
```{r}
qca2.ds <- ds %>% select(T, C, N, E, H, A, L, S)
qca2.direxp.high = "1,1,1,1,1,1,1"
qca2.direxp.low = "0,0,0,0,0,0,0"
qca2.cutoff = ".8"
qca2.method = "QMC"
```
...
### Select variables and directional expectations
```{r}
qca3.ds <- ds %>% select(G, C, E, H, A)
qca3.direxp.high = "0,0,0,0"
qca3.direxp.low = "1,1,1,1"
qca3.cutoff = ".8"
qca3.method = "QMC"
```
...
#### truth table
```{r}
qca3.high.tt <- truthTable(
  data = qca3.ds,
  outcome = "G",
  show.cases = TRUE,
  incl.cut = qca3.cutoff)
qca3.high.tt
```

#### Conservative solution
```{r}
qca3.high.conserv <- minimize(qca3.high.tt, details = TRUE, method = qca3.method)
qca3.high.conserv
```

#### Parsimonious solution
```{r}
qca3.high.parsim <- minimize(qca3.high.tt, include = "?", details = TRUE, method = qca3.method)
qca3.high.parsim
```

#### Intermediate solution
```{r}
qca3.high.interm <- minimize(qca3.high.tt, include = "?", dir.exp = qca3.direxp.high, details = TRUE, method = qca3.method)
qca3.high.interm
```
...
#### Conservative solution
```{r}
qca3.low.conserv <- minimize(qca3.low.tt, details = TRUE, method = qca3.method)
qca3.low.conserv
```

#### Parsimonious solution
```{r}
qca3.low.parsim <- minimize(qca3.low.tt, include = "?", details = TRUE, method = qca3.method)
qca3.low.parsim
```

#### Intermediate solution
```{r}
qca3.low.interm <- minimize(qca3.low.tt, include = "?", dir.exp = qca3.direxp.low, details = TRUE, method = qca3.method)
qca3.low.interm
```